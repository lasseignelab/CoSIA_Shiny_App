...
setGeneric("getConvert", function(object) standardGeneric("getConvert"))
...
setMethod("getConvert", signature(object = "CoSIAn"), function(object) { # user's input of the function
  ...
  for(index in seq(length(output_species))){
    ...
    if(is.na(species_data)){
      Filter_I_Species <- switch(input_species,
                                 h_sapiens = {
                                   species_data<-h_sapiens(input_id,input,output_ids,output_species[index], tool, ortholog_database)
                                 },
                                 m_musculus = {
                                   species_data<-m_musculus(input_id,input,output_ids,output_species[index], tool, ortholog_database)
                                 },
                                 r_norvegicus = {
                                   species_data<-r_norvegicus(input_id,input,output_ids,output_species[index], tool, ortholog_database)
                                 },
                                 d_rerio = {
                                   species_data<-d_rerio(input_id,input,output_ids,output_species[index], tool, ortholog_database)
                                 },
                                 c_elegans = {
                                   species_data<-c_elegans(input_id,input,output_ids,output_species[index], tool, ortholog_database)
                                 },
                                 d_melanogaster = {
                                   species_data<-d_melanogaster(input_id,input,output_ids,output_species[index], tool, ortholog_database)
                                 },
      ...
      Filter_I_Species <- switch(input_species,
                                 h_sapiens = {
                                   species_data<-full_join(species_data, h_sapiens(input_id,input,output_ids,output_species[index], tool, ortholog_database))
                                 },
                                 m_musculus = {
                                   species_data<-m_musculus(input_id,input,output_ids,output_species[index], tool, ortholog_database)
                                 },
